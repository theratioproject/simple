					...
					if ( simple_list_isstring(pVar,SIMPLE_VAR_VALUE) ) {
						if ( strcmp(simple_list_getstring(pVar,SIMPLE_VAR_VALUE),"NULL") == 0 ) {
							...
							if ( simple_list_getlist(vm->pActiveMem,simple_list_getsize(vm->pActiveMem)) == pVar ) {
								...
								simple_list_deletelastitem_gc(vm->pSimpleState,vm->pActiveMem);
							}
							return ;
						}
					}
...
void simple_vm_loadaddress ( VM *vm )
{
	if ( simple_vm_findvar(vm, SIMPLE_VM_IR_READC  ) == 0 ) {
		...
		simple_list_setint_gc(vm->pSimpleState,(List *) SIMPLE_VM_STACK_READP,SIMPLE_VAR_PRIVATEFLAG,vm->nPrivateFlag);
	}
	...
	if ( vm->nVarScope == SIMPLE_VARSCOPE_GLOBAL ) {
		...
		simple_item_setpointer_gc(vm->pSimpleState,SIMPLE_VM_IR_ITEM(1),SIMPLE_VM_STACK_READP);
	}
		...
		simple_item_setpointer_gc(vm->pSimpleState,SIMPLE_VM_IR_ITEM(3),SIMPLE_VM_STACK_READP);
		simple_item_setint_gc(vm->pSimpleState,SIMPLE_VM_IR_ITEM(4),simple_list_getint(vm->aScopeID,simple_list_getsize(vm->aScopeID)));
		#if SIMPLE_SHOWICFINAL
		SIMPLE_VM_IR_PARACOUNT = SIMPLE_VM_IR_PARACOUNT + 2 ;
		simple_list_addpointer_gc(vm->pSimpleState,SIMPLE_VM_IR_LIST,SIMPLE_VM_STACK_READP);
		simple_list_addint_gc(vm->pSimpleState,SIMPLE_VM_IR_LIST,simple_list_getint(vm->aScopeID,simple_list_getsize(vm->aScopeID)));
		#endif
	}
	/* Add Result Scope to aLoadAddressScope Array */
	simple_list_addint_gc(vm->pSimpleState,vm->aLoadAddressScope,vm->nVarScope);
}
...
void simple_vm_assignment ( VM *vm )
{
	...
	if ( SIMPLE_VM_STACK_PREVOBJTYPE == SIMPLE_OBJTYPE_SUBSTRING ) {
		...
		if ( (SIMPLE_VM_STACK_ISSTRING) && (vm->nBeforeEqual <= 1 ) ) {
			...
			SIMPLE_VM_STACK_POP ;
			if ( vm->nBeforeEqual == 0 ) {
				simple_list_setint_gc(vm->pSimpleState,pVar, SIMPLE_VAR_TYPE ,SIMPLE_VM_NUMBER);
				simple_list_setdouble_gc(vm->pSimpleState,pVar, SIMPLE_VAR_VALUE , nNum1);
			} else {
				/* Check NULL Variable */
				if ( simple_list_getint(pVar,SIMPLE_VAR_TYPE) == SIMPLE_VM_NULL ) {
					simple_vm_error2(vm,SIMPLE_VM_ERROR_USINGNULLVARIABLE,simple_list_getstring(pVar,SIMPLE_VAR_NAME));
			...
			if ( (SIMPLE_VM_STACK_OBJTYPE == SIMPLE_OBJTYPE_VARIABLE) || (SIMPLE_VM_STACK_OBJTYPE ==SIMPLE_OBJTYPE_LISTITEM) ) {
				...
				*/
				...
				pVar = (List *) SIMPLE_VM_STACK_READP ;
				SIMPLE_VM_STACK_POP ;
				simple_list_setint_gc(vm->pSimpleState,pVar, SIMPLE_VAR_TYPE ,SIMPLE_VM_LIST);
				simple_list_setlist_gc(vm->pSimpleState,pVar,SIMPLE_VAR_VALUE);
				...
				simple_list_delete_gc(vm->pSimpleState,pList);
			}
		} else {
			simple_vm_error(vm,SIMPLE_VM_ERROR_BADVALUES);
		}
...
void simple_vm_inc ( VM *vm )
{
	...
	pVar = (List *) SIMPLE_VM_STACK_READP ;
	SIMPLE_VM_STACK_POP ;
	simple_list_setdouble_gc(vm->pSimpleState,pVar,SIMPLE_VAR_VALUE,simple_list_getdouble(pVar,SIMPLE_VAR_VALUE) + 1);
}
...
void simple_vm_freestack ( VM *vm )
{
	...
	simple_list_deleteallitems_gc(vm->pSimpleState,vm->aLoadAddressScope);
	/* In the class region */
	...
	if ( ( simple_list_getsize(vm->pFuncCallList) == 0 ) && (vm->nInsideBraceFlag == 0) ) {
		...
		simple_list_deleteallitems_gc(vm->pSimpleState,vm->pTempMem);
	} else {
...
void simple_vm_setreference ( VM *vm )
{
	...
	*/
	...
	/* Copy by reference */
	simple_list_setint_gc(vm->pSimpleState,pList,SIMPLE_VAR_TYPE,SIMPLE_VM_POINTER);
	simple_list_setpointer_gc(vm->pSimpleState,pList,SIMPLE_VAR_VALUE,pPointer);
	simple_list_setint_gc(vm->pSimpleState,pList,SIMPLE_VAR_PVALUETYPE,nType);
...
void simple_vm_list_copy ( VM *vm,List *pNewList, List *pList )
{
	...
	for ( x = 1 ; x <= simple_list_getsize(pList) ; x++ ) {
		if ( simple_list_isint(pList,x) ) {
			simple_list_addint_gc(vm->pSimpleState,pNewList,simple_list_getint(pList,x));
		}
		else if ( simple_list_isdouble(pList,x) ) {
			simple_list_adddouble_gc(vm->pSimpleState,pNewList,simple_list_getdouble(pList,x));
		}
		else if ( simple_list_isstring(pList,x) ) {
			simple_list_addstring2_gc(vm->pSimpleState,pNewList,simple_list_getstring(pList,x),simple_list_getstringsize(pList,x));
		}
		else if ( simple_list_ispointer(pList,x) ) {
			simple_list_addpointer_gc(vm->pSimpleState,pNewList,simple_list_getpointer(pList,x));
		}
		else if ( simple_list_islist(pList,x) ) {
			pNewList2 = simple_list_newlist_gc(vm->pSimpleState,pNewList);
	...
	if ( simple_list_getsize(pList) == SIMPLE_CPOINTER_LISTSIZE ) {
		if ( simple_list_ispointer(pList,SIMPLE_CPOINTER_POINTER)  && simple_list_isstring(pList,SIMPLE_CPOINTER_TYPE) && simple_list_isint(pList,SIMPLE_CPOINTER_STATUS) ) {
			...
			if ( simple_list_getint(pList,SIMPLE_CPOINTER_STATUS) == SIMPLE_CPOINTERSTATUS_NOTCOPIED ) {
				...
				simple_list_setint_gc(vm->pSimpleState,pList,SIMPLE_CPOINTER_STATUS,SIMPLE_CPOINTERSTATUS_COPIED);
				simple_list_setint_gc(vm->pSimpleState,pNewList,SIMPLE_CPOINTER_STATUS,SIMPLE_CPOINTERSTATUS_COPIED);
				...
				if ( simple_list_findpointer(vm->aCPointers,simple_list_getpointer(pList,SIMPLE_CPOINTER_POINTER)) == 0 ) {
					simple_list_addpointer_gc(vm->pSimpleState,vm->aCPointers,simple_list_getpointer(pList,SIMPLE_CPOINTER_POINTER));
				}
			}
				...
				simple_list_setint_gc(vm->pSimpleState,pList,SIMPLE_CPOINTER_STATUS,SIMPLE_CPOINTERSTATUS_NOTCOPIED);
				simple_list_setint_gc(vm->pSimpleState,pNewList,SIMPLE_CPOINTER_STATUS,SIMPLE_CPOINTERSTATUS_NOTCOPIED);
			}
		}
	}
}
...
void simple_vm_beforeequallist ( VM *vm,List *pVar,double nNum1 )
{
	...
	if ( simple_list_isdouble(pVar,SIMPLE_VAR_VALUE) ) {
		if ( vm->nBeforeEqual == 1 ) {
			simple_list_setdouble_gc(vm->pSimpleState,pVar, SIMPLE_VAR_VALUE ,simple_list_getdouble(pVar,SIMPLE_VAR_VALUE) + nNum1);
			...
			simple_list_setdouble_gc(vm->pSimpleState,pVar, SIMPLE_VAR_VALUE , fmod(simple_list_getdouble(pVar,SIMPLE_VAR_VALUE), nNum1));
		} else if ( vm->nBeforeEqual == 6 ) {
			simple_list_setdouble_gc(vm->pSimpleState,pVar, SIMPLE_VAR_VALUE , (int) simple_list_getdouble(pVar,SIMPLE_VAR_VALUE) & (int) nNum1);
...
void simple_vm_beforeequalitem ( VM *vm,Item *pItem,double nNum1 )
{
	...
	if ( simple_item_isdouble(pItem) ) {
		if ( vm->nBeforeEqual == 1 ) {
			simple_item_setdouble_gc(vm->pSimpleState,pItem ,simple_item_getdouble(pItem) + nNum1);
		} else if ( vm->nBeforeEqual == 2 ) {
			simple_item_setdouble_gc(vm->pSimpleState,pItem ,simple_item_getdouble(pItem) - nNum1);
		} else if ( vm->nBeforeEqual == 3 ) {
			simple_item_setdouble_gc(vm->pSimpleState,pItem ,simple_item_getdouble(pItem) * nNum1);
			...
			simple_item_setdouble_gc(vm->pSimpleState,pItem ,fmod(simple_item_getdouble(pItem) , nNum1));
		} else if ( vm->nBeforeEqual == 6 ) {
			simple_item_setdouble_gc(vm->pSimpleState,pItem ,(int) simple_item_getdouble(pItem) & (int) nNum1);
		} else if ( vm->nBeforeEqual == 7 ) {
			simple_item_setdouble_gc(vm->pSimpleState,pItem ,(int) simple_item_getdouble(pItem) | (int) nNum1);
		} else if ( vm->nBeforeEqual == 8 ) {
			simple_item_setdouble_gc(vm->pSimpleState,pItem ,(int) simple_item_getdouble(pItem) ^ (int) nNum1);
	...
	if ( SIMPLE_VM_STACK_ISPOINTER ) {
		if ( SIMPLE_VM_STACK_OBJTYPE == SIMPLE_OBJTYPE_VARIABLE ) {
			...
			if ( simple_list_isdouble(pList,SIMPLE_VAR_VALUE) ) {
				simple_list_setdouble_gc(vm->pSimpleState,pList,SIMPLE_VAR_VALUE,simple_list_getdouble(pList,SIMPLE_VAR_VALUE)+1);
				return ;
			}
		}
		else if ( SIMPLE_VM_STACK_OBJTYPE == SIMPLE_OBJTYPE_LISTITEM ) {
			pItem = (Item *) SIMPLE_VM_STACK_READP ;
			if ( simple_item_isdouble(pItem) ) {
				simple_item_setdouble_gc(vm->pSimpleState,pItem,simple_item_getdouble(pItem)+1);
				return ;
			}
		}
	}
