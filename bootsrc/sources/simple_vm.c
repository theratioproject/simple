...
int simple_vm_eval ( VM *pVM,const char *cStr )
{
	...
	simple_list_addstsimple_gc(pVM->pRingState,pVM->pRingState->pRingFilesList,"executeCode");
	simple_list_addstsimple_gc(pVM->pRingState,pVM->pRingState->pRingFilesStack,"executeCode");
	...
	if ( nCont == 1 ) {
		...
		simple_vm_error(pVM,"Error in executeCode!");
