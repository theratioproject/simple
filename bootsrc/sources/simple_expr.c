...
int simple_parser_expr ( Parser *pParser )
{
	...
	if ( simple_parser_logicnot(pParser) ) {
		...
		#endif
		while ( simple_parser_iskeyword(pParser,KEYWORD_AND) || simple_parser_isoperator(pParser,"&&")  || simple_parser_iskeyword(pParser,KEYWORD_OR) || simple_parser_isoperator(pParser,"||") ) {
			if ( simple_parser_iskeyword(pParser,KEYWORD_AND) || simple_parser_isoperator(pParser,"&&") ) {
				/* Generate Code */
...
int simple_parser_logicnot ( Parser *pParser )
{
	...
	if ( simple_parser_iskeyword(pParser,KEYWORD_NOT) || simple_parser_isoperator2(pParser,OP_NOT) ) {
		simple_parser_nexttoken(pParser);
		SIMPLE_PARSER_IGNORENEWLINE ;
...
int simple_parser_factor ( Parser *pParser,int *nFlag )
{
	...
	*nFlag = 0 ;
	...
	if ( simple_parser_iskeyword(pParser,KEYWORD_NEW) ) {
		simple_parser_nexttoken(pParser);
		SIMPLE_PARSER_IGNORENEWLINE ;
		/* Generate Code */
