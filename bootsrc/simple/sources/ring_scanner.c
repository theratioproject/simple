...
void display_tokens ( Scanner *pScanner )
{
	...
	print_line();
	printf( "\n" ) ;
	for ( x = 0 ; x <= ring_list_getsize(pScanner->Tokens) ; x++ ) {
		pList = ring_list_getlist(pScanner->Tokens,x);
		token_type = ring_list_getint(pList,RING_SCANNER_TOKENTYPE) ;
		token_name = ring_list_getstring(pList,RING_SCANNER_TOKENVALUE) ;
		switch ( token_type ) {
