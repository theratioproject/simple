...
void ring_vm_refmeta_locals ( void *pPointer )
{
	...
	for ( x = 1 ; x <= ring_list_getsize(pList) ; x++ ) {
		...
		ring_list_addstring_gc(((VM *) pPointer)->pRingState,pList2,ring_list_getstring(pList3,SIMPLE_VAR_NAME));
	}
...
void ring_vm_refmeta_globals ( void *pPointer )
{
	...
	for ( x = SIMPLE_VM_INTERNALGLOBALSCOUNT + 1 ; x <= ring_list_getsize(pList) ; x++ ) {
		pList3 = ring_list_getlist(pList,x);
		ring_list_addstring_gc(((VM *) pPointer)->pRingState,pList2,ring_list_getstring(pList3,SIMPLE_VAR_NAME));
	}
	SIMPLE_API_RETLIST(pList2);
...
void ring_vm_refmeta_functions ( void *pPointer )
{
	...
	for ( x = 1 ; x <= ring_list_getsize(pVM->pFunctionsMap) ; x++ ) {
		...
		ring_list_addstring_gc(((VM *) pPointer)->pRingState,pList,ring_list_getstring(pList2,SIMPLE_FUNCMAP_NAME));
	}
...
void ring_vm_refmeta_islocal ( void *pPointer )
{
	...
	if ( SIMPLE_API_GETSTRING(1) ) {
		...
		for ( x = 1 ; x <= ring_list_getsize(pList) ; x++ ) {
			...
			if ( strcmp(ring_list_getstring(pList2,SIMPLE_VAR_NAME),cStr) == 0 ) {
...
void ring_vm_refmeta_isfunction ( void *pPointer )
{
	...
	if ( SIMPLE_API_GETSTRING(1) ) {
		...
		for ( x = 1 ; x <= ring_list_getsize(pList) ; x++ ) {
			pList2 = ring_list_getlist(pList,x);
			if ( strcmp(ring_list_getstring(pList2,SIMPLE_FUNCMAP_NAME),cStr) == 0 ) {
				SIMPLE_API_RETNUMBER(1);
				return ;
			}
		}
...
void ring_vm_refmeta_packages ( void *pPointer )
{
	...
	for ( x = 1 ; x <= ring_list_getsize(pVM->pPackagesMap) ; x++ ) {
		...
		ring_list_addstring_gc(((VM *) pPointer)->pRingState,pList,ring_list_getstring(pList2,SIMPLE_PACKAGENAME));
	}
	SIMPLE_API_RETLIST(pList);
}
...
void ring_vm_refmeta_ispackage ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISSTRING(1) ) {
		...
		for ( x = 1 ; x <= ring_list_getsize(pList) ; x++ ) {
			pList2 = ring_list_getlist(pList,x);
			if ( strcmp(ring_list_getstring(pList2,SIMPLE_PACKAGENAME),cStr) == 0 ) {
				SIMPLE_API_RETNUMBER(1);
				return ;
			}
		}
...
void ring_vm_refmeta_classes ( void *pPointer )
{
	...
	for ( x = 1 ; x <= ring_list_getsize(pVM->pClassesMap) ; x++ ) {
		...
		ring_list_addstring_gc(((VM *) pPointer)->pRingState,pList,ring_list_getstring(pList2,SIMPLE_CLASSMAP_CLASSNAME));
	}
	SIMPLE_API_RETLIST(pList);
}
...
void ring_vm_refmeta_isclass ( void *pPointer )
{
	...
	if ( SIMPLE_API_GETSTRING(1) ) {
		...
		for ( x = 1 ; x <= ring_list_getsize(pList) ; x++ ) {
			pList2 = ring_list_getlist(pList,x);
			if ( strcmp(ring_list_getstring(pList2,SIMPLE_CLASSMAP_CLASSNAME),cStr) == 0 ) {
				SIMPLE_API_RETNUMBER(1);
				return ;
			}
		}
	...
	if ( SIMPLE_API_GETSTRING(1) ) {
		...
		for ( x = 1 ; x <= ring_list_getsize(pList) ; x++ ) {
			...
			if ( strcmp(ring_list_getstring(pList2,SIMPLE_PACKAGENAME),cStr) == 0 ) {
				...
				pList2 = ring_list_getlist(pList2,SIMPLE_CLASSESLIST) ;
				/* We can use the variable x for the loop again because we have return */
				for ( x = 1 ; x <= ring_list_getsize(pList2) ; x++ ) {
					ring_list_addstring_gc(((VM *) pPointer)->pRingState,pList3,ring_list_getstring(ring_list_getlist(pList2,x),SIMPLE_CLASSMAP_CLASSNAME));
				}
	...
	if ( SIMPLE_API_GETSTRING(1) &&  SIMPLE_API_GETSTRING(2) ) {
		...
		for ( x = 1 ; x <= ring_list_getsize(pList) ; x++ ) {
			...
			if ( strcmp(ring_list_getstring(pList2,SIMPLE_PACKAGENAME),cStr) == 0 ) {
				...
				/* We can use the variable x for the loop again because we have return */
				for ( x = 1 ; x <= ring_list_getsize(pList2) ; x++ ) {
					if ( strcmp(ring_list_getstring(ring_list_getlist(pList2,x),SIMPLE_CLASSMAP_CLASSNAME),cStr2)==0 ) {
						SIMPLE_API_RETNUMBER(1);
						return ;
					}
				}
...
void ring_vm_refmeta_classname ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISLIST(1) ) {
		...
		if ( ring_vm_oop_isobject(pList) ) {
			cStr = ring_list_getstring((List *) ring_list_getpointer(pList,SIMPLE_OBJECT_CLASSPOINTER),SIMPLE_CLASSMAP_CLASSNAME);
	...
	if ( SIMPLE_API_ISLIST(1) ) {
		...
		if ( ring_vm_oop_isobject(pList) ) {
			...
			for ( x = 3 ; x <= ring_list_getsize(pList) ; x++ ) {
				ring_list_addstring_gc(((VM *) pPointer)->pRingState,pList2,ring_list_getstring(ring_list_getlist(pList,x),SIMPLE_VAR_NAME));
			}
			SIMPLE_API_RETLIST(pList2);
		} else {
			SIMPLE_API_ERROR(SIMPLE_API_BADPARATYPE);
...
void ring_vm_refmeta_methods ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISLIST(1) ) {
		pList = SIMPLE_API_GETLIST(1) ;
		if ( ring_vm_oop_isobject(pList) ) {
			pList = (List *) ring_list_getpointer(pList,SIMPLE_OBJECT_CLASSPOINTER);
			pList = ring_list_getlist(pList,SIMPLE_CLASSMAP_METHODSLIST);
			pList2 = SIMPLE_API_NEWLIST ;
			for ( x = 1 ; x <= ring_list_getsize(pList) ; x++ ) {
				ring_list_addstring_gc(((VM *) pPointer)->pRingState,pList2,ring_list_getstring(ring_list_getlist(pList,x),SIMPLE_FUNCMAP_NAME));
			}
			SIMPLE_API_RETLIST(pList2);
		} else {
			SIMPLE_API_ERROR(SIMPLE_API_BADPARATYPE);
...
void ring_vm_refmeta_isattribute ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISLIST(1) && SIMPLE_API_ISSTRING(2) ) {
		...
		if ( ring_vm_oop_isobject(pList) ) {
			...
			pList = ring_list_getlist(pList,SIMPLE_OBJECT_OBJECTDATA);
			for ( x = 3 ; x <= ring_list_getsize(pList) ; x++ ) {
				if ( strcmp(cStr,ring_list_getstring(ring_list_getlist(pList,x),SIMPLE_VAR_NAME))==0 ) {
					SIMPLE_API_RETNUMBER(1);
					return ;
				}
			}
...
void ring_vm_refmeta_ismethod ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISLIST(1) ) {
		...
		if ( SIMPLE_API_ISSTRING(2) ) {
			cStr = SIMPLE_API_GETSTRING(2) ;
			ring_string_lower(cStr);
			SIMPLE_API_RETNUMBER(ring_vm_oop_ismethod((VM *) pPointer,pList,cStr));
		}
	} else {
		SIMPLE_API_ERROR(SIMPLE_API_BADPARATYPE);
	}
...
void ring_vm_refmeta_isprivateattribute ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISLIST(1) && SIMPLE_API_ISSTRING(2) ) {
		...
		if ( ring_vm_oop_isobject(pList) ) {
			...
			for ( x = 3 ; x <= ring_list_getsize(pList) ; x++ ) {
				if ( strcmp(cStr,ring_list_getstring(ring_list_getlist(pList,x),SIMPLE_VAR_NAME))==0 ) {
					if ( ring_list_getint(ring_list_getlist(pList,x),SIMPLE_VAR_PRIVATEFLAG) == 1 ) {
						SIMPLE_API_RETNUMBER(1);
					}
					else {
						SIMPLE_API_RETNUMBER(0);
...
void ring_vm_refmeta_addmethod ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISLIST(1) && SIMPLE_API_ISSTRING(2) && SIMPLE_API_ISSTRING(3) ) {
		pList = SIMPLE_API_GETLIST(1) ;
		if ( ring_vm_oop_isobject(pList) ) {
			pList = (List *) ring_list_getlist(pList,SIMPLE_OBJECT_CLASSPOINTER);
			pList = (List *) ring_list_getlist(pList,SIMPLE_CLASSMAP_METHODSLIST);
			...
			for ( x = 1 ; x <= ring_list_getsize(pVM->pFunctionsMap) ; x++ ) {
				...
				if ( strcmp(ring_list_getstring(pList2,SIMPLE_FUNCMAP_NAME),cStr) == 0 ) {
					...
					ring_list_setstring_gc(((VM *) pPointer)->pRingState,pList3,SIMPLE_FUNCMAP_NAME,ring_string_lower(SIMPLE_API_GETSTRING(2)));
...
void ring_vm_refmeta_getattribute ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISLIST(1) && SIMPLE_API_ISSTRING(2) ) {
		...
		if ( ring_vm_oop_isobject(pList) ) {
			...
			ring_string_lower(cStr);
			for ( x = 1 ; x <= ring_list_getsize(pList) ; x++ ) {
				if ( strcmp(ring_list_getstring(ring_list_getlist(pList,x),SIMPLE_VAR_NAME),cStr) == 0 ) {
					pList = ring_list_getlist(pList,x) ;
					if ( ring_list_isnumber(pList,SIMPLE_VAR_VALUE) ) {
						SIMPLE_API_RETNUMBER(ring_list_getdouble(pList,SIMPLE_VAR_VALUE));
					}
					else if ( ring_list_isstring(pList,SIMPLE_VAR_VALUE) ) {
						SIMPLE_API_RETSTRING2(ring_list_getstring(pList,SIMPLE_VAR_VALUE),ring_list_getstringsize(pList,SIMPLE_VAR_VALUE));
					}
					else if ( ring_list_islist(pList,SIMPLE_VAR_VALUE) ) {
						SIMPLE_API_RETLIST(ring_list_getlist(pList,SIMPLE_VAR_VALUE));
					}
					return ;
				}
			}
			...
			for ( x = 1 ; x <= ring_list_getsize(pList) ; x++ ) {
				if ( strcmp(ring_list_getstring(ring_list_getlist(pList,x),SIMPLE_VAR_NAME),cStr) == 0 ) {
					...
					if ( SIMPLE_API_ISNUMBER(3) ) {
						ring_list_setdouble_gc(((VM *) pPointer)->pRingState,pList,SIMPLE_VAR_VALUE,SIMPLE_API_GETNUMBER(3));
					}
					else if ( SIMPLE_API_ISSTRING(3) ) {
						ring_list_setstring2_gc(((VM *) pPointer)->pRingState,pList,SIMPLE_VAR_VALUE,SIMPLE_API_GETSTRING(3),SIMPLE_API_GETSTRINGSIZE(3));
					}
					else if ( SIMPLE_API_ISLIST(3) ) {
						ring_list_setlist_gc(((VM *) pPointer)->pRingState,pList,SIMPLE_VAR_VALUE);
						pList = ring_list_getlist(pList,SIMPLE_VAR_VALUE);
						ring_list_deleteallitems_gc(((VM *) pPointer)->pRingState,pList);
						ring_list_copy(pList,SIMPLE_API_GETLIST(3));
					}
					return ;
				}
			}
	...
	*/
	...
	if ( SIMPLE_API_ISSTRING(1) && SIMPLE_API_ISSTRING(2) ) {
		...
		for ( x = 1 ; x <= ring_list_getsize(pVM->pClassesMap) ; x++ ) {
			...
			if ( strcmp(ring_list_getstring(pList,SIMPLE_CLASSMAP_CLASSNAME),cStr) == 0 ) {
				/* Check if the class is imported from a Package */
				if ( ring_list_getsize(pList) == SIMPLE_CLASSMAP_IMPORTEDCLASSSIZE ) {
					pList = ring_list_getlist(pList,SIMPLE_CLASSMAP_POINTERTOLISTOFCLASSINSIDEPACKAGE);
				}
				pList2 = ring_list_getlist(pList,SIMPLE_CLASSMAP_METHODSLIST) ;
				break ;
			}
		}
		if ( pList2 == NULL ) {
		...
		for ( x = 1 ; x <= ring_list_getsize(pVM->pClassesMap) ; x++ ) {
			pList = ring_list_getlist(pVM->pClassesMap,x) ;
			if ( strcmp(ring_list_getstring(pList,SIMPLE_CLASSMAP_CLASSNAME),cStr2) == 0 ) {
				/* Check if the class is imported from a Package */
				if ( ring_list_getsize(pList) == SIMPLE_CLASSMAP_IMPORTEDCLASSSIZE ) {
					pList = ring_list_getlist(pList,SIMPLE_CLASSMAP_POINTERTOLISTOFCLASSINSIDEPACKAGE);
				}
				pList3 = ring_list_getlist(pList,SIMPLE_CLASSMAP_METHODSLIST) ;
				break ;
			}
		}
		if ( pList3 == NULL ) {
...
void ring_vm_refmeta_packagename ( void *pPointer )
{
	...
	SIMPLE_API_RETSTRING(ring_string_get(pVM->pPackageName));
}
/* VM */
...
void ring_vm_refmeta_ringvmtracefunc ( void *pPointer )
{
	VM *pVM  ;
	pVM = (VM *) pPointer ;
	SIMPLE_API_RETSTRING(ring_string_get(pVM->pTrace));
}
...
void ring_vm_refmeta_ringvmscopescount ( void *pPointer )
{
	...
	SIMPLE_API_RETNUMBER(ring_list_getsize(pVM->pMem) - 1);
}
...
void ring_vm_refmeta_ringvmcallfunc ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISSTRING(1) ) {
		...
		pString = ring_string_new_gc(((VM *) pPointer)->pRingState,SIMPLE_API_GETSTRING(1));
