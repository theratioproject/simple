	...
	if ( pVM->nListStart == 1 ) {
		...
		if ( (pVM->nFuncExecute > 0)  || ( nCont == 1 ) ) {
			if ( ring_list_getsize(pVM->pFuncCallList) > 0 ) {
				...
				ring_vm_newtempvar(pVM, SIMPLE_TEMP_VARIABLE ,pVM->pTempMem);
			}
		...
		if ( nType == SIMPLE_OBJTYPE_VARIABLE ) {
			...
			ring_list_deleteallitems_gc(pVM->pRingState,ring_list_getlist(pVar,SIMPLE_VAR_VALUE));
			ring_list_addpointer_gc(pVM->pRingState,pVM->pNestedLists,ring_list_getlist(pVar,SIMPLE_VAR_VALUE));
		}
...
void ring_vm_listitem ( VM *pVM )
{
	...
	if ( SIMPLE_VM_STACK_ISSTRING ) {
		cStr1 = ring_string_new_gc(pVM->pRingState,SIMPLE_VM_STACK_READC);
		SIMPLE_VM_STACK_POP ;
		...
		if ( SIMPLE_VM_STACK_OBJTYPE == SIMPLE_OBJTYPE_VARIABLE ) {
			...
			SIMPLE_VM_STACK_POP ;
			pList2 = ring_list_getlist(pList2,SIMPLE_VAR_VALUE);
...
void ring_vm_loadindexaddress ( VM *pVM )
{
	...
	if ( SIMPLE_VM_STACK_ISNUMBER ) {
			...
			if ( SIMPLE_VM_STACK_OBJTYPE == SIMPLE_OBJTYPE_VARIABLE ) {
				...
				if ( ring_list_islist(pVar,SIMPLE_VAR_VALUE) ) {
					...
					if ( nNum1 < 1 || nNum1 > ring_list_getsize(pVar) ) {
						ring_vm_error(pVM,SIMPLE_VM_ERROR_INDEXOUTOFRANGE);
						return ;
					}
					pItem = ring_list_getitem(pVar,nNum1);
					SIMPLE_VM_STACK_PUSHPVALUE(pItem);
				}
				else if ( ring_list_isstring(pVar,SIMPLE_VAR_VALUE) ) {
					SIMPLE_VM_STACK_POP ;
					if ( ring_list_getint(pVar,SIMPLE_VAR_TYPE) == SIMPLE_VM_NULL ) {
						ring_vm_error(pVM,SIMPLE_VM_ERROR_USINGNULLVARIABLE);
						return ;
					}
					...
					SIMPLE_VM_STACK_POP ;
					ring_vm_error(pVM,SIMPLE_VM_ERROR_OBJECTISNOTLIST);
					return ;
				}
			}
			else if ( SIMPLE_VM_STACK_OBJTYPE == SIMPLE_OBJTYPE_LISTITEM ) {
		...
		pString = ring_string_new_gc(pVM->pRingState,SIMPLE_VM_STACK_READC);
		SIMPLE_VM_STACK_POP ;
...
void ring_vm_listpushv ( VM *pVM )
{
	...
	if ( ring_item_gettype(pItem) == ITEMTYPE_STRING ) {
		...
		pVM->nSP++ ;
		SIMPLE_VM_STACK_SETCVALUE2(ring_string_get(ring_item_getstring(pItem)),ring_string_size(ring_item_getstring(pItem)));
	}
		...
		SIMPLE_VM_STACK_PUSHNVALUE(ring_item_getnumber(pItem));
	}
	...
	if ( ring_list_getsize(pVar) > 0 ) {
		for ( x = 1 ; x <= ring_list_getsize(pVar) ; x++ ) {
			if ( ring_list_islist(pVar,x) ) {
				...
				if ( ring_list_getsize(pList)  >= SIMPLE_LISTHASH_SIZE ) {
					if ( ring_list_isstring(pList,SIMPLE_LISTHASH_KEY) ) {
						cStr2 = ring_list_getstring(pList,SIMPLE_LISTHASH_KEY);
						if ( ring_vm_strcmpnotcasesensitive(cStr,cStr2)  == 0 ) {
							pItem = ring_list_getitem(pList,SIMPLE_LISTHASH_VALUE);
							SIMPLE_VM_STACK_PUSHPVALUE(pItem);
							SIMPLE_VM_STACK_OBJTYPE = SIMPLE_OBJTYPE_LISTITEM ;
							return ;
						}
