...
SIMPLE_API void ring_vm_loadcfunctions ( RingState *pRingState )
{
	...
	*/
	...
	ring_vm_funcregister("ringvm_read",ring_vmlib_give);
...
SIMPLE_API void ring_vm_api_retlist ( void *pPointer,List *pList )
{
	...
	ring_list_setint_gc(((VM *) pPointer)->pRingState,pList3,SIMPLE_VAR_TYPE,SIMPLE_VM_LIST);
	ring_list_setlist_gc(((VM *) pPointer)->pRingState,pList3,SIMPLE_VAR_VALUE);
...
SIMPLE_API void * ring_vm_api_varptr ( void *pPointer,const char  *cStr,const char *cStr2 )
{
	...
	*/
	...
	SIMPLE_VM_STACK_POP ;
	if ( ring_list_getint(pList,SIMPLE_VAR_TYPE) == SIMPLE_VM_NUMBER ) {
		pItem = ring_list_getitem(pList,SIMPLE_VAR_VALUE);
	...
	else if ( ring_list_getint(pList,SIMPLE_VAR_TYPE) == SIMPLE_VM_STRING ) {
		pItem = ring_list_getitem(pList,SIMPLE_VAR_VALUE);
...
{
	if ( SIMPLE_API_ISLIST(x) ) {
		return ring_vm_api_iscpointerlist(SIMPLE_API_GETLIST(x)) ;
	}
	return 0 ;
}
...
{
	if ( SIMPLE_API_ISLIST(x) ) {
		return ring_vm_oop_isobject(SIMPLE_API_GETLIST(x)) ;
	}
	return 0 ;
}
...
{
	if ( ring_list_getpointer(pList,SIMPLE_CPOINTER_POINTER) == ring_list_getpointer(pList2,SIMPLE_CPOINTER_POINTER) ) {
		return 1 ;
	}
	else {
		return 0 ;
...
SIMPLE_API int ring_vm_api_ispointer ( void *pPointer,int x )
{
	...
	Item *pItem  ;
	pVM = (VM *) pPointer ;
	pList = ring_list_getlist(SIMPLE_API_PARALIST,x) ;
	if ( ring_list_ispointer(pList,SIMPLE_VAR_VALUE) ) {
		return 1 ;
	}
	else if ( ring_list_isstring(pList,SIMPLE_VAR_VALUE) ) {
		/* Treat NULL Strings as NULL Pointers - so we can use NULL instead of NULLPOINTER() */
		if ( strcmp(ring_list_getstring(pList,SIMPLE_VAR_VALUE),"") == 0 ) {
			/* Create the list for the NULL Pointer */
			ring_list_setint_gc(((VM *) pPointer)->pRingState,pList,SIMPLE_VAR_TYPE,SIMPLE_VM_POINTER);
			...
			ring_list_setpointer_gc(((VM *) pPointer)->pRingState,pList,SIMPLE_VAR_VALUE,pItem);
			ring_list_setint_gc(((VM *) pPointer)->pRingState,pList,SIMPLE_VAR_PVALUETYPE,SIMPLE_OBJTYPE_LISTITEM);
			/* The variable value will be a list contains the pointer */
...
void ring_vmlib_len ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISSTRING(1) ) {
		...
		if ( ring_vm_oop_isobject(SIMPLE_API_GETLIST(1)) == 0 ) {
			SIMPLE_API_RETNUMBER(ring_list_getsize(SIMPLE_API_GETLIST(1)));
		}
		else {
...
void ring_vmlib_add ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISLIST(1) ) {
		...
		if ( SIMPLE_API_ISSTRING(2) ) {
			ring_list_addstring2_gc(pVM->pRingState,pList,SIMPLE_API_GETSTRING(2),SIMPLE_API_GETSTRINGSIZE(2));
			SIMPLE_API_RETSTRING2(SIMPLE_API_GETSTRING(2),SIMPLE_API_GETSTRINGSIZE(2));
		}
		else if ( SIMPLE_API_ISNUMBER(2) ) {
			ring_list_adddouble_gc(pVM->pRingState,pList,SIMPLE_API_GETNUMBER(2));
...
void ring_vmlib_input ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISNUMBER(1) ) {
		...
		nSize = ring_vm_stringtonum((VM *) pPointer,SIMPLE_API_GETSTRING(1));
	} else {
		SIMPLE_API_ERROR(SIMPLE_API_BADPARATYPE);
		return ;
	}
...
void ring_vmlib_filename ( void *pPointer )
{
	...
	if ( pVM->nInClassRegion ) {
		SIMPLE_API_RETSTRING(pVM->cFileNameInClassRegion);
		return ;
	}
	if ( (pVM->nFuncExecute2 > 0) && (ring_list_getsize(pVM->pFuncCallList)>0) ) {
		...
		*/
		...
		if ( (nPos > 0) && (nPos <= ring_list_getsize(pVM->pFuncCallList)) ) {
			...
			if ( ring_list_getsize(pList) >= SIMPLE_FUNCCL_FILENAME ) {
				SIMPLE_API_RETSTRING((char *) ring_list_getpointer(pList,SIMPLE_FUNCCL_FILENAME ));
			}
		}
		return ;
	}
	SIMPLE_API_RETSTRING(pVM->cFileName);
}
...
void ring_vmlib_type ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISSTRING(1) ) {
		...
		pList = SIMPLE_API_GETLIST(1) ;
		SIMPLE_API_RETSTRING(ring_list_getstring(pList,SIMPLE_CPOINTER_TYPE));
	}
...
void ring_vmlib_isobject ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISLIST(1) ) {
		if ( ring_vm_oop_isobject(SIMPLE_API_GETLIST(1) ) == 1 ) {
			SIMPLE_API_RETNUMBER(1);
			return ;
		}
	}
...
void ring_vmlib_number ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISSTRING(1) ) {
		x = ring_vm_stringtonum((VM *) pPointer,SIMPLE_API_GETSTRING(1));
		SIMPLE_API_RETNUMBER(x);
	} else {
		SIMPLE_API_ERROR(SIMPLE_API_BADPARATYPE);
	}
...
void ring_vmlib_list2str ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISLIST(1) ) {
		...
		SIMPLE_API_RETSTRING(ring_string_get(pString));
...
void ring_vmlib_copy ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISSTRING(1) ) {
		if ( SIMPLE_API_ISNUMBER(2) ) {
			...
			for ( x = 1 ; x <= nSize ; x++ ) {
				ring_string_add2_gc(((VM *) pPointer)->pRingState,pString,cStr,SIMPLE_API_GETSTRINGSIZE(1));
			}
	...
	*/
	...
	if ( nTransform > 0 ) {
		...
		while ( cString != NULL ) {
			...
			ring_string_add2_gc(((VM *) pPointer)->pRingState,pString,cStr3,SIMPLE_API_GETSTRINGSIZE(3));
			nMark = nPos + nSize2 -1 ;
			/* Search */
			if ( nTransform == 1 ) {
...
void ring_vmlib_lower ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISSTRING(1) ) {
		SIMPLE_API_RETSTRING2(ring_string_lower2(SIMPLE_API_GETSTRING(1),SIMPLE_API_GETSTRINGSIZE(1)),SIMPLE_API_GETSTRINGSIZE(1));
	} else {
		SIMPLE_API_ERROR(SIMPLE_API_BADPARATYPE);
	}
}
...
void ring_vmlib_upper ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISSTRING(1) ) {
		SIMPLE_API_RETSTRING2(ring_string_upper2(SIMPLE_API_GETSTRING(1),SIMPLE_API_GETSTRINGSIZE(1)),SIMPLE_API_GETSTRINGSIZE(1));
	} else {
		SIMPLE_API_ERROR(SIMPLE_API_BADPARATYPE);
	}
}
...
void ring_vmlib_varptr ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISSTRING(1) && SIMPLE_API_ISSTRING(2) ) {
		cStr = SIMPLE_API_GETSTRING(1);
		cStr2 = SIMPLE_API_GETSTRING(2);
		SIMPLE_API_RETCPOINTER(ring_vm_api_varptr(pPointer,cStr,cStr2),cStr2);
	} else {
		SIMPLE_API_ERROR(SIMPLE_API_BADPARATYPE);
	}
}
...
void ring_vmlib_space ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISNUMBER(1) ) {
		pString = ring_string_new2_gc(((VM *) pPointer)->pRingState,"",SIMPLE_API_GETNUMBER(1));
		SIMPLE_API_RETSTRING2(ring_string_get(pString),SIMPLE_API_GETNUMBER(1));
		ring_string_delete_gc(((VM *) pPointer)->pRingState,pString);
	} else {
		SIMPLE_API_ERROR(SIMPLE_API_BADPARATYPE);
	}
...
void ring_vmlib_ptrcmp ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISCPOINTER(1) && SIMPLE_API_ISCPOINTER(2) ) {
		pList = SIMPLE_API_GETLIST(1) ;
		pList2 = SIMPLE_API_GETLIST(2) ;
		SIMPLE_API_RETNUMBER(ring_vm_api_cpointercmp(pList,pList2));
	} else {
		SIMPLE_API_ERROR(SIMPLE_API_BADPARATYPE);
	}
}
...
{
	SIMPLE_API_RETCPOINTER(ring_state_init(),"RINGSTATE");
}
...
void ring_vmlib_state_runcode ( void *pPointer )
{
	...
	ring_state_runcode((RingState *) SIMPLE_API_GETCPOINTER(1,"RINGSTATE"),SIMPLE_API_GETSTRING(2));
}
...
void ring_vmlib_state_delete ( void *pPointer )
{
	...
	ring_state_delete((RingState *) SIMPLE_API_GETCPOINTER(1,"RINGSTATE"));
}
...
void ring_vmlib_state_runfile ( void *pPointer )
{
	...
	ring_state_runfile((RingState *) SIMPLE_API_GETCPOINTER(1,"RINGSTATE"),SIMPLE_API_GETSTRING(2));
}
...
void ring_vmlib_state_findvar ( void *pPointer )
{
	...
	pList = ring_state_findvar((RingState *) SIMPLE_API_GETCPOINTER(1,"RINGSTATE"),SIMPLE_API_GETSTRING(2));
	SIMPLE_API_RETLIST(pList);
}
...
void ring_vmlib_state_newvar ( void *pPointer )
{
	...
	pList = ring_state_newvar((RingState *) SIMPLE_API_GETCPOINTER(1,"RINGSTATE"),SIMPLE_API_GETSTRING(2));
	SIMPLE_API_RETLIST(pList);
}
...
void ring_vmlib_state_runobjectfile ( void *pPointer )
{
	...
	ring_state_runobjectfile((RingState *) SIMPLE_API_GETCPOINTER(1,"RINGSTATE"),SIMPLE_API_GETSTRING(2));
}
...
void ring_vmlib_state_setvar ( void *pPointer )
{
	...
	if ( SIMPLE_API_ISSTRING(3) ) {
		...
		ring_list_setstring2_gc(pVM->pRingState,pList, SIMPLE_VAR_VALUE , SIMPLE_API_GETSTRING(3),SIMPLE_API_GETSTRINGSIZE(3));
	}
	else if ( SIMPLE_API_ISNUMBER(3) ) {
		ring_list_setint_gc(pVM->pRingState,pList, SIMPLE_VAR_TYPE ,SIMPLE_VM_NUMBER);
		ring_list_setdouble_gc(pVM->pRingState,pList, SIMPLE_VAR_VALUE ,SIMPLE_API_GETNUMBER(3));
	}
		...
		pList3 = ring_list_getlist(pList,SIMPLE_VAR_VALUE);
...
{
	SIMPLE_API_RETCPOINTER(ring_state_new(),"RINGSTATE");
}
...
void ring_vmlib_state_mainfile ( void *pPointer )
{
	...
	pRingState = (RingState *) SIMPLE_API_GETCPOINTER(1,"RINGSTATE") ;
