...
:buildsimpledllexe	
	...
	if %THERE_IS_VS%=="true" (
		...
		cl.exe /D_USRDLL /D_WINDLL /LD /Tc !SIMPLE_C_FILES! /MT /link /DLL /OUT:%~dp0\..\simple\dist\heloworld.dll
		call:deletetempfiles *.obj *.exp
...
:getsimplecfiles
	if exist "..\simple\sources\" (
		SET SIMPLE_C_FILES="%~dp0\..\simple\sources\simple_api.c %~dp0\..\simple\sources\simple_codegen.c"
	) else (
