...
:buildsimpledllexe	
	...
	if exist "%~dp0\..\simple\dist"  (
		...
		REM rmdir /S /Q %~dp0\..\simple\dist
	)
	if %THERE_IS_VS%=="true" (
		call:getsimplecfiles
		echo yea Visual Studio and flies are !SIMPLE_C_FILES!
		mkdir %~dp0\..\simple\dist
		rem cl.exe /D_USRDLL /D_WINDLL /TC !SIMPLE_C_FILES! /MT /I %~dp0\..\simple\includes\ /link /DLL /OUT:%~dp0\..\simple\dist\simple.dll
		call:deletetempfiles *.obj *.exp *.cod
		REM move *.lib %~dp0\..\simple\dist
		if exist "..\simple\dist\simple.lib" (
			cl.exe /Fo.\.\ /Fe.\.\ /GA /MT /TC %~dp0\..\simple\simple.c %~dp0\..\simple\dist\simple.dll
			call:deletetempfiles *.obj *.exp *.cod
		)
		exit /b 0
	) else (
