...
:builddynamicmodules
	...
	if !THERE_IS_VS!=="true" (
		...
		cl.exe /D_USRDLL /D_WINDLL /TC !DYNAMIC_MODULE_FILES! /MT /I %~dp0\..\simple\includes\ /link /DLL /OUT:%~dp0\..\simple\dist\simple.dll
	) else (
		if exist "..\modules" (
			cd "..\modules"
...
:getdynamicmodulefiles
	if exist "..\modules\dynamic_modules\" (
		if %1=="archiver" (
			SET SIMPLE_C_FILES="%~dp0\..\modules\dynamic_modules\archiver\*.c"
		)
	) else (
		call:repocurrupterror dynamic_modules
	)
