...
:resolvedependencies
	...
	if !BUILD_ARC!=="x64" (
		if exist "..\modules\dynamic_modules\security\bin\ssleay64.dll" (
			call:movedependencytobin ssleay64.dll ..\modules\dynamic_modules\security\bin\ssleay64.dll
		) else (
			call:dependencieserror ssleay32.dll
		)
	) else (
		if exist "..\modules\dynamic_modules\security\bin\ssleay32.dll" (
			call:movedependencytobin ssleay32.dll ..\modules\dynamic_modules\security\bin\ssleay32.dll
		) else (
			call:dependencieserror ssleay32.dll
		)
	)
...
:movedependencytobin
	...
	if !EXEC_TYPE!=="install" (
		copy %2 !INSTALLATION_FOLDER!\%VERSION%\bin\
	)
	if !EXEC_TYPE!=="debug" (
		copy %2 ..\..\%SIMPLE_DEBUG_VERSION%\bin
	)
