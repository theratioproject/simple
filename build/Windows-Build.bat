...
:buildsingleenvironment 
	echo environment:build: building %1 %3
	if !EXEC_TYPE!=="install" (
		!SIMPLE_EXECUTABLE! !BAKE_EXECUTABLE! %2 !INSTALLATION_FOLDER!\%VERSION%\bin\%1.sim
	)
	if !EXEC_TYPE!=="debug" (
		echo "!SIMPLE_EXECUTABLE! !BAKE_EXECUTABLE! %2 ..\..\%SIMPLE_DEBUG_VERSION%\bin\%1.sim"
		!SIMPLE_EXECUTABLE! !BAKE_EXECUTABLE! %2 ..\..\%SIMPLE_DEBUG_VERSION%\bin\%1.sim
	)
