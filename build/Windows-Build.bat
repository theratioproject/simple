...
:buildsimpledllexe	
	...
	if exist "%~dp0\..\simple\dist"  (
		...
		rmdir /S /Q %~dp0\..\simple\dist
	)
	if %THERE_IS_VS%=="true" (
		call:getsimplecfiles
		echo yea Visual Studio and flies are !SIMPLE_C_FILES!
		mkdir %~dp0\..\simple\dist
		cl.exe /D_USRDLL /D_WINDLL /TC !SIMPLE_C_FILES! /MT /I %~dp0\..\simple\includes\ /link /DLL /OUT:%~dp0\..\simple\dist\simple.dll legacy_stdio_definitions.lib
