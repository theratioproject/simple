		...
		*/
		...
		if ( simple_list_getint(list,SIMPLE_BLOCKCL_TYPE) == SIMPLE_BLOCKTYPE_SCRIPT ) {
			...
			*/
			...
			printf("<td title='%s' bgcolor='#eeeeec' ><a href='file:///%s' target='__blank' >%s</a></td>",simple_list_getstring(vm->sState->files_list,1),simple_list_getstring(vm->sState->files_list,1),file_real_name(cFile) ) ;
			lBlockCall = 1 ;
		}
		else {
			printf( "In %s ",file_real_name(simple_list_getstring(list,SIMPLE_BLOCKCL_NAME)) ) ;
